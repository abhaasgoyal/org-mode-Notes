...
# Evaluate all files in the source block not only the snippet while loading org in babel mode
...
- Whenever you are starting into org ref\
   ...
   #+END_EXAMPLE
- For scripting languages :results output should be there as the header argument for printing every line
